...
/**
 * Stores the parameters of the LED: port and pin configured in main.h
 */
typedef struct {
	GPIO_TypeDef *port;
	uint16_t pin;
...
/// Buffer for matrix rows configuration
pin_config_t rows[] = { { ROW_1_GPIO_Port, ROW_1_Pin }, { ROW_2_GPIO_Port,
...
/// Buffer for matrix columns configuration
pin_config_t cols[] = {
...
/// Index of current col in a cycle
...
/**
 * @brief  Set col state, turn on/off.
 * @param  state: Type states_t: TURN_ON, TURN_OFF
 * @retval None
 */
...
/**
 * @brief  Turn on each LED on the matrix through 100 ms, turning it on completely.
 * @param  None
 * @retval None
 */
